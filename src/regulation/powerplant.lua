...
local PRESSURE_TRESHOLD = 0.97  --enter standby mode. During very low steam consumption the pressure
                                --oscillates around the target with a deviation of (PRESSURE_TRESHOLD - target_r).
...
while true do
  ...
  if (get_pressure() >= PRESSURE_TRESHOLD and mode_r == "balance") then
    print('Reactor - standby')
    mode_r = "standby"
  elseif (get_pressure() <= (2*target_r) - PRESSURE_TRESHOLD and mode_r == "standby") then
    print('Reactor - balance')
    mode_r = "balance"
  end
  ...
  for i = 1, NUMBER_OF_TURBINES do
    ...
    elseif speed >= (target_t[i] - INDUCTION_TRESHOLD) and not t[i].getInductorEngaged() then
      print(string.format("Turbine %d - engaged", i))
      t[i].setInductorEngaged(true)
    elseif speed < (target_t[i] - INDUCTION_TRESHOLD) and t[i].getInductorEngaged() then
      print(string.format("Turbine %d - disengaged", i))
      t[i].setInductorEngaged(false)
    end
  end
